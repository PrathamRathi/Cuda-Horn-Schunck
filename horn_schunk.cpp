...
void computeDerivatives(const Mat& im1, const Mat& im2, Mat& ix, Mat& iy, Mat& it) {
    ...
    Mat kernelY = (Mat_<double>(2, 2) << -0.25, -0.25, 0.25, 0.25); // Kernel for dy
    ...
    filter2D(im1_d, fx1, -1, kernelX, Point(-1, -1), 0, BORDER_DEFAULT);
    filter2D(im2_d, fx2, -1, kernelX, Point(-1, -1), 0, BORDER_DEFAULT);
    ix = fx1 + fx2;

    filter2D(im1_d, fy1, -1, kernelY, Point(-1, -1), 0, BORDER_DEFAULT);
    filter2D(im2_d, fy2, -1, kernelY, Point(-1, -1), 0, BORDER_DEFAULT);
    iy = fy1 + fy2;

    filter2D(im2_d, ft1, -1, -kernelT, Point(-1, -1), 0, BORDER_DEFAULT);
    filter2D(im1_d, ft2, -1, kernelT, Point(-1, -1), 0, BORDER_DEFAULT);
    it = ft1 + ft2;
}
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        ...
        for (int y = 0; y < frame1.rows; ++y) {
            for (int x = 0; x < frame1.cols; ++x) {
                ...
                // // Limit flow values to prevent explosion
                // flowX.at<double>(y, x) = std::max(-100.0, std::min(100.0, u));
                // flowY.at<double>(y, x) = std::max(-100.0, std::min(100.0, v));
            }
        }
    }
}
