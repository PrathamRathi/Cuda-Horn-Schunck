...
void exchangeHalos(double* data_with_halos, int nx, int local_ny, int rank, int size, MPI_Comm comm) {
    if (size == 1) return;

    // Calculate source/destination ranks
    int prev_rank = (rank > 0) ? rank - 1 : MPI_PROC_NULL;
    int next_rank = (rank < size - 1) ? rank + 1 : MPI_PROC_NULL;
    
    // Point to the actual data (after first halo)
    double* data = data_with_halos + nx;
    ...
    if (prev_rank != MPI_PROC_NULL) {
        MPI_Irecv(data_with_halos, nx, MPI_DOUBLE, prev_rank, 0, comm, &requests[req_count++]);
    }
    if (next_rank != MPI_PROC_NULL) {
        MPI_Irecv(data + local_ny * nx, nx, MPI_DOUBLE, next_rank, 1, comm, &requests[req_count++]);
    }
    
    // Then post sends
    if (prev_rank != MPI_PROC_NULL) {
        MPI_Isend(data, nx, MPI_DOUBLE, prev_rank, 1, comm, &requests[req_count++]);
    }
    if (next_rank != MPI_PROC_NULL) {
        MPI_Isend(data + (local_ny - 1) * nx, nx, MPI_DOUBLE, next_rank, 0, comm, &requests[req_count++]);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (provided < MPI_THREAD_FUNNELED) {
        cerr << "Warning: The MPI implementation does not support the required thread level\n";
    }
    ...
    if (size < 1) {
        cerr << "Error: At least one MPI process is required\n";
        MPI_Finalize();
        return 1;
    }
    ...
    if (argc != 4) {
        if (rank == 0) {
            cerr << "Usage: " << argv[0] << " <input_image1> <input_image2> <output_name>\n";
        }
        MPI_Finalize();
        return 1;
    }
    ...
    for (int size : test_sizes) {
        ...
        // Calculate proper local dimensions with halos
        int local_ny = (ny + size - 1) / size;  // Ceiling division to handle non-divisible sizes
        int my_start = rank * local_ny;
        int my_end = min((rank + 1) * local_ny, ny);
        local_ny = my_end - my_start;  // Actual number of rows for this rank

        // Allocate host memory with halos
        size_t local_size = (local_ny + 2) * nx * sizeof(double);  // +2 for halos
        double *uHost_with_halos = nullptr;
        double *vHost_with_halos = nullptr;

        GPU_ERROR = cudaHostAlloc(&uHost_with_halos, local_size, cudaHostAllocDefault);
        if (GPU_ERROR != cudaSuccess) {
            cerr << "Rank " << rank << ": Failed to allocate uHost_with_halos" << endl;
            MPI_Abort(MPI_COMM_WORLD, 1);
            return 1;
        }

        GPU_ERROR = cudaHostAlloc(&vHost_with_halos, local_size, cudaHostAllocDefault);
        if (GPU_ERROR != cudaSuccess) {
            cudaError_t freeError = cudaFreeHost(uHost_with_halos);
            if (freeError != cudaSuccess) {
                cerr << "Failed to free uHost_with_halos: " << cudaGetErrorString(freeError) << endl;
            }
            cerr << "Rank " << rank << ": Failed to allocate vHost_with_halos" << endl;
            MPI_Abort(MPI_COMM_WORLD, 1);
            return 1;
        }

        // Point to the interior of the arrays (after the first halo)
        uHost = uHost_with_halos + nx;
        vHost = vHost_with_halos + nx;
        ...
        if (GPU_ERROR != cudaSuccess) {
            cudaError_t freeError = cudaFree(IxDevice);
            if (freeError != cudaSuccess) {
                cerr << "Failed to free IxDevice: " << cudaGetErrorString(freeError) << endl;
            }
        ...
        if (GPU_ERROR != cudaSuccess) {
            ...
            freeError = cudaFree(IyDevice);
            if (freeError != cudaSuccess) {
                cerr << "Failed to free IyDevice: " << cudaGetErrorString(freeError) << endl;
            }
        ...
        // Initialize streams with error checking
        for (int i = 0; i < NUM_STREAMS; i++) {
            error = cudaStreamCreate(&streams[i]);
            if (error != cudaSuccess) {
                cerr << "Failed to create stream " << i << ": " << cudaGetErrorString(error) << endl;
                return -1;
            }
        }

        // Main computation loop with proper error handling
        for(int iter = 0; iter < iterations; iter++) {
            // Copy from device to host (interior points only)
            error = cudaMemcpy(uHost, u_d, local_ny * nx * sizeof(double), cudaMemcpyDeviceToHost);
            if (error != cudaSuccess) {
                cerr << "Rank " << rank << ": Failed to copy u_d to host" << endl;
                break;
            }
            
            error = cudaMemcpy(vHost, v_d, local_ny * nx * sizeof(double), cudaMemcpyDeviceToHost);
            if (error != cudaSuccess) {
                cerr << "Rank " << rank << ": Failed to copy v_d to host" << endl;
                break;
            }
            
            // Exchange halos
            exchangeHalos(uHost_with_halos, nx, local_ny, rank, size, MPI_COMM_WORLD);
            exchangeHalos(vHost_with_halos, nx, local_ny, rank, size, MPI_COMM_WORLD);
            
            // Copy entire local domain including halos back to device
            error = cudaMemcpy(u_d - nx, uHost_with_halos, (local_ny + 2) * nx * sizeof(double), 
                              cudaMemcpyHostToDevice);
            if (error != cudaSuccess) {
                cerr << "Rank " << rank << ": Failed to copy data back to device (u)" << endl;
                break;
            }
            
            error = cudaMemcpy(v_d - nx, vHost_with_halos, (local_ny + 2) * nx * sizeof(double), 
                              cudaMemcpyHostToDevice);
            if (error != cudaSuccess) {
                cerr << "Rank " << rank << ": Failed to copy data back to device (v)" << endl;
                break;
            }
            
            // Launch kernel with proper dimensions
            dim3 grid((nx + block.x - 1) / block.x, (local_ny + block.y - 1) / block.y);
            fused_horn_schunk<<<grid, block>>>(u_d, v_d, IxDevice, IyDevice, ItDevice, 
                                              alpha, nx, local_ny);
        }
        ...
        for (int i = 0; i < NUM_STREAMS; i++) {
            ...
            if (error != cudaSuccess) {
                cerr << "Failed to destroy stream " << i << ": " << cudaGetErrorString(error) << endl;
            }
        }

        // Free device memory with proper error handling
        if (IxDevice) {
            error = cudaFree(IxDevice);
            if (error != cudaSuccess) {
                cerr << "Failed to free IxDevice: " << cudaGetErrorString(error) << endl;
            }
            IxDevice = nullptr;
        }

        if (IyDevice) {
            error = cudaFree(IyDevice);
            if (error != cudaSuccess) {
                cerr << "Failed to free IyDevice: " << cudaGetErrorString(error) << endl;
            }
            IyDevice = nullptr;
        }

        if (ItDevice) {
            error = cudaFree(ItDevice);
            if (error != cudaSuccess) {
                cerr << "Failed to free ItDevice: " << cudaGetErrorString(error) << endl;
            }
            ItDevice = nullptr;
        }
        ...
        if (u_d) {
            error = cudaFree(u_d);
            if (error != cudaSuccess) {
                cerr << "Failed to free u_d: " << cudaGetErrorString(error) << endl;
            }
            u_d = nullptr;
        }
        ...
        if (v_d) {
            error = cudaFree(v_d);
            if (error != cudaSuccess) {
                cerr << "Failed to free v_d: " << cudaGetErrorString(error) << endl;
            }
            v_d = nullptr;
        }
    ...
    // Finalize MPI
    if (uHost_with_halos) {
        cudaError_t error = cudaFreeHost(uHost_with_halos);
        if (error != cudaSuccess) {
            cerr << "Failed to free uHost_with_halos: " << cudaGetErrorString(error) << endl;
        }
    }
    if (vHost_with_halos) {
        cudaError_t error = cudaFreeHost(vHost_with_halos);
        if (error != cudaSuccess) {
            cerr << "Failed to free vHost_with_halos: " << cudaGetErrorString(error) << endl;
        }
    }
